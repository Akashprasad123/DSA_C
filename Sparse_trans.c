...
#define MAX_SIZE 50
    ...
    void Transpose(int mat[][MAX_SIZE], int trans[][MAX_SIZE],int m, int n){
        int i,j;
        for(i=0; i<m; i++){
            for(j=0; j<n; j++){
                trans[i][j]=mat[j][i];
    ...
    void main(){
        ...
        mat_in(a,m,n);
        print_mat(a,m,n);
        Transpose(a,trans,m,n);
        Sparse_mat(trans,m,n,r,c,val,&count);
        printf("Sparce matrix representation of transpose :\n");
